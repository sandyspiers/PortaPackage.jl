...
for (path, name) in source_dict
    dest_path = joinpath(dest, "dev", name)
    isdir(dest_path) && rm(dest_path; recursive=true, force=true)
    mkpath(dest_path)
    # Use robocopy instead of cp() — Julia's sendfile fails on Windows for large trees.
    # robocopy exit codes 0-7 are all success variants (8+ indicate failures).
    proc = run(ignorestatus(`robocopy $path $dest_path /E /NFL /NDL /NJH /NJS /R:0 /W:0`))
    proc.exitcode < 8 || error("robocopy failed with exit code $(proc.exitcode) copying $path to $dest_path")
    path = dest_path
